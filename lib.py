...
class Datum:
  ...
  def __init__(self, datumSurfaceForm, datumLabel):
    if isinstance(datumSurfaceForm, str):
      self.datumTokens = word_tokenize(datumSurfaceForm)
    else:
      self.datumTokens = word_tokenize(str(datumSurfaceForm))
...
def show_mistakes(label, guessedTrue, guessedFalse):
  false_positives = [str(x) for x in guessedTrue if x.answer() != label]
  false_negatives = [str(x) for x in guessedFalse if x.answer() == label]
